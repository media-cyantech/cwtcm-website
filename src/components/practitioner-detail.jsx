// ⚠️ 由 scripts/esmify.mjs 从 ../../practitioner-detail.jsx 转换而来，之后有人工改动，不要重跑覆盖。
// 与根目录原文件的差异仅限：ESM 导入导出 + 语言由 useStrings() 注入（原为自由全局 STRINGS）。

import { useState as usePdState } from 'react';
...
// 原来读模块顶层的 PD_IS_ZH。同一类隐患，改成显式接语言。
const pdFont = (isZh, zhStack) => (isZh ? (zhStack || 'var(--font-sans-zh)') : 'var(--font-sans)');

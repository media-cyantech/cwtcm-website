...
const { useState: useStateCDD } = React;
const CDD_IS_ZH = (typeof STRINGS !== 'undefined') && STRINGS.lang === 'zh';
...
const CDDHeroPlaceholder = ({ kind, alt }) => (
  <div role="img" aria-label={alt} style={{
    ...
    <div style={{
      ...
      {CDD_IS_ZH ? '图片即将上线' : 'Photography coming soon'}
    </div>
  </div>
);
        ...
        <div className="cdd-hero-grid" style={{
          ...
          <div className="cdd-hero-image" style={{
            ...
            <CDDHeroPlaceholder kind={d.kind}
              alt={`${d.name} ${CDD_IS_ZH ? '场景图（占位）' : '— image coming soon'}`} />
          </div>
        </div>
      </div>
    </section>
...
Object.assign(window, { ConditionDetailPage });
